...
export function buildInterior(lot: Lot, floor: 1 | 2): InteriorBuilt {
  ...
  const rooms = lotPlan(lot).filter((r) => r.floor === floor);
  const g = new THREE.Group();
  g.position.set(lot.x, floor === 2 ? arch.h * 0.48 : 0, lot.z);
  g.rotation.y = lot.rot;

  const shellW = arch.w * 1.38;
  const shellD = arch.d * 1.18;
  ...
  const window = new THREE.Mesh(
    new THREE.PlaneGeometry(Math.min(2.4, arch.w * 0.7), 1.15),
  ...
  for (const room of rooms) {
    ...
    dot.position.set(room.x, 0.95, room.z);
    ...
    stem.position.set(room.x, 0.45, room.z);
    g.add(stem);
  }
...
function addFurniture(g: THREE.Group, room: PlanRoom) {
  ...
  if (kind === "live" || kind === "studio") {
    g.add(box(Math.min(1.9, w * 0.7), 0.52, 0.62, SAND, x, 0.42, z + d * 0.18));
    g.add(box(0.85, 0.32, 0.85, TEAK, x, 0.32, z - d * 0.05));
    g.add(box(Math.min(1.6, w * 0.55), 0.04, Math.min(1.2, d * 0.4), 0xc45c3e, x, 0.14, z));
  }
  if (kind === "kitchen") {
    g.add(box(w * 0.86, 0.9, 0.5, 0x3d4f4c, x, 0.55, z + d * 0.28));
    g.add(box(w * 0.5, 0.08, 0.48, STONE, x, 1.02, z + d * 0.28));
    g.add(box(0.7, 0.75, 0.7, TEAK, x - w * 0.15, 0.5, z - d * 0.1));
  }
  if (kind === "bed") {
    g.add(box(Math.min(1.5, w * 0.7), 0.42, Math.min(2.0, d * 0.7), LINEN, x, 0.38, z));
    g.add(box(0.35, 0.4, 0.35, TEAK, x + w * 0.32, 0.32, z + d * 0.22));
    g.add(box(Math.min(1.5, w * 0.7), 0.12, 0.18, LAGOON, x, 0.62, z - d * 0.28));
  }
  if (kind === "bath") {
    g.add(box(Math.min(1.1, w * 0.45), 1.4, Math.min(1.1, d * 0.7), 0xd9e2e0, x + w * 0.22, 0.8, z));
    g.add(box(0.7, 0.7, 0.4, STONE, x - w * 0.22, 0.48, z + d * 0.1));
  }
  if (kind === "studio") {
...
function addPartitions(g: THREE.Group, room: PlanRoom, shellW: number, shellD: number) {
  const wallH = 2.55;
  const t = 0.08;
  const color = 0xefe8dc;
  const left = room.x - room.w / 2;
  const right = room.x + room.w / 2;
  const front = room.z + room.d / 2;
  const back = room.z - room.d / 2;
  const inner = Math.abs(left) < shellW * 0.45 && Math.abs(right) < shellW * 0.45;

  if (inner || Math.abs(left) > 0.4) {
    const door = 0.7;
    g.add(box(t, wallH, Math.max(0.2, room.d - door), color, left, wallH / 2, room.z - door * 0.2));
  }
  if (Math.abs(back + shellD * 0.5) > 0.35) {
    g.add(box(Math.max(0.3, room.w * 0.7), wallH, t, color, room.x, wallH / 2, back));
  }
  void front;
  void right;
}
